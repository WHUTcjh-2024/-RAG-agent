...
export function ProductCard({ product, index, selected, onCompare, onAdd }: Props) {
  return (
    <article className="product-card group animate-reveal" style={{ animationDelay: `${Math.min(index, 8) * 45}ms` }}>
      <div className="relative aspect-[3/4] overflow-hidden bg-[#e8e4dc]">
        <img src={productImage(product)} alt={product.prod_name} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 ease-out group-hover:scale-[1.035]" />
      ...
      <div className="pt-3">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate text-sm font-medium">{product.prod_name}</h3>
