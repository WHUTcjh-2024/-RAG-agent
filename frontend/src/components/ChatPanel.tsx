...
const prompts = ["适合夏天通勤的白色衬衫", "简约但有质感的约会穿搭", "帮我找类似图片的款式"];
  ...
  return (
    <section className="flex min-h-[600px] flex-col border border-ink/10 bg-paper">
      <div className="border-b border-ink/10 px-5 py-4 text-xs leading-5 text-muted">
        描述场景与偏好，或上传参考图片。顾问只会推荐目录中真实存在的商品。
      </div>
            ...
            <div key={message.id} className={message.role === "user" ? "ml-8" : "mr-4"}>
              ...
              <div className={message.role === "user" ? "bg-ink px-4 py-3 text-sm leading-6 text-white" : "border-l-2 border-accent/50 pl-4 text-sm leading-6"}>
                {message.imagePreview && (
                  <img src={message.imagePreview} className="mb-3 h-28 w-24 object-cover" alt="上传参考" />
                )}
                {message.content}
      ...
      <div className="border-t border-ink/10 p-4">
        {preview && (
          <div className="mb-3 flex items-center gap-3 bg-canvas p-2">
            <img src={preview} className="h-14 w-12 object-cover" alt="预览" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-medium">{image?.name}</p>
              <p className="mt-0.5 text-[10px] text-muted">将用于视觉相似检索</p>
            </div>
        ...
        <div className="flex items-end gap-2 border border-ink/15 bg-white p-2 focus-within:border-ink/40">
          <label className="cursor-pointer p-2 text-muted transition-colors hover:text-ink" aria-label="上传图片">
          ...
          <textarea
            ...
            rows={2}
            placeholder="描述你想找的单品或场景…"
            className="max-h-28 min-h-11 flex-1 resize-none bg-transparent px-1 py-2 text-sm outline-none placeholder:text-muted/70"
          />
          <button aria-label="发送" onClick={submit} disabled={streaming || (!value.trim() && !image)} className="grid h-10 w-10 shrink-0 place-items-center bg-accent text-white transition-colors hover:bg-accent-dark disabled:cursor-not-allowed disabled:bg-ink/15">
