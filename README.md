...
> 我做的是一个多模态电商导购 Agent。它不是让大模型直接推荐商品，而是先用 MiniLM 和 CLIP 分别建立文本、图像索引，再融合结构化属性和真实交易热度做排序。Agent 通过 LangChain 工具完成搜索、对比、加购和结算，LLM 只负责工具选择及候选集内的推荐理由，因此结果可控、可追溯。系统支持 SQLite 多轮记忆、SSE 流式输出、无 LLM 回退、Docker 部署和完整 CI；当前在 5,000 个真实商品样本上建立了 62 条回归评测用例。
